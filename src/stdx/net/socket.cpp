...
void stdx::_NetworkIOService::send(socket_t sock, const char* data, const socket_size_t& size, std::function<void(network_send_event, std::exception_ptr)> callback)
{
	...
	char* buffer = (char*) stdx::_calloc(sizeof(char), size);
	if (buffer == nullptr)
	{
		delete context_ptr;
		callback(stdx::network_send_event(), std::make_exception_ptr(std::bad_alloc()));
	...
	*call = [callback](network_io_context* context_ptr, std::exception_ptr error)
	{
		if (error)
		{
			stdx::_free(context_ptr->buffer.buf);
			delete context_ptr;
...
#endif // DEBUG
#else
	char* buf = (char*) stdx::_calloc(size, sizeof(char));
	if (buf == nullptr)
	{
		callback(stdx::network_send_event(), std::make_exception_ptr(std::bad_alloc()));
		return;
...
void stdx::_NetworkIOService::recv(socket_t sock, const socket_size_t& size, std::function<void(network_recv_event, std::exception_ptr)> callback)
{
	...
	context_ptr->this_socket = sock;
	char* buf = (char*) stdx::_calloc(sizeof(char), size);
	if (buf == nullptr)
	{
		delete context_ptr;
		callback(stdx::network_recv_event(), std::make_exception_ptr(std::bad_alloc()));
...
#else
	char* buf = (char*) ::stdx::_malloc(size);
	if (buf == nullptr)
	{
		callback(stdx::network_send_event(), std::make_exception_ptr(std::bad_alloc()));
		return;
...
stdx::task<stdx::network_send_event> stdx::_Socket::send(const char* data, const socket_size_t& size)
{
	if (!m_io_service)
	{
		throw std::logic_error("this io service has been stdx::_free");
	}
...
void stdx::_Socket::recv_utill(const socket_size_t& size, std::function<bool(stdx::task_result<stdx::network_recv_event>)> call)
{
...
void stdx::_Socket::recv_utill_error(const socket_size_t& size, std::function<void(stdx::network_recv_event)> call, std::function<void(std::exception_ptr)> err_handler)
{
